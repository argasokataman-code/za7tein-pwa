...
Onboarding merchant: registrasi toko → approval Super Admin → deposit 3,50 JOD → kredit Founding 5 JOD → aktif → onboarding kurir. Layar signup merchant sudah ada di repo (`/merchant/signup`, `/merchant/pending`), tetapi sisanya layar mock. Dasar bisnis: PRD aktif §5C (deposit COD) + §Insentif Founding Merchant (`docs/product/prd/versions/irbid-mvp-v2-2026-09-21/source.md:145-193`) + `I-1`/`I-2` + `C-12`; lihat `../BUSINESS.md`. Repo ini front-end saja (AGENTS.md §1).
...
**Lane Layar merchant (signup · form toko · kurir):** Signup merchant (email + password lewat `merchantSignInSchema`/`merchantSignUpSchema` di `src/lib/schemas.ts`, nomor HP opsional) → user dibuat `type: merchant` → form profil toko (description, foto D1, `deliveryConfig`: radius + `feeByDistance` ATAU area + `feeByArea`) → setelah aktif: layar toko aktif (buka/tutup) dan onboarding kurir.
...
- Onboarding kurir hanya untuk karyawan merchant setelah toko aktif: rekrut off-app, maks `MAX_COURIERS_PER_MERCHANT` (3, `src/data/merchant.ts`), SA/merchant assign `courier.merchant = merchant.id`.
