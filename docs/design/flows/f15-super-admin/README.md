...
Panel admin platform yang **dikelola CS** — role ke-4 di repo ini (slug flow tetap `f15-super-admin` untuk kesinambungan). **Sudah di-develop (basis UI)**: konsol PWA `/admin/*` dengan shell 430px sama seperti role lain. Rencana & keputusan implementasi ada di `docs/plan-admin.md`; spec ini tetap sumber gambaran alur. **Super Admin adalah role terpisah** (website penuh non-PWA, belum dibangun) menurut keputusan PO 2026-09-23 — sebagian `C-12` ("portal CS merangkap super admin") digantikan, lihat `docs/product/prd/decision-irbid-mvp.md`. **Cakupan SA sudah ditetapkan PO 2026-09-23** (master zona, role & permission, audit trail, laporan pajak, saldo keuntungan platform, kill switch, monitoring ledger detail) — tapi route-nya belum dibuat (keputusan PO: dokumen dulu). Panel CS tetap menjalankan approval tenant, putusan sengketa level-1, dan blacklist COD; SA mengaudit + menangani banding. PRD meminta approval tenant, konfigurasi kuota tier, dan master zona (AGENTS.md §9); skema draft menambah deposit approval, blacklist COD, liability dashboard, SLA escalation, dan dispute queue. Sumber gambaran: `docs/product/schema-draft-v1.md` + PRD `irbid-mvp-v2-2026-09-21`. Repo ini front-end saja — konsol dan aksinya adalah layar mock (AGENTS.md §1).
...
**Lane Konsol panel admin/CS (dashboard):** Queue onboarding (`merchant.tenantStatus: pending`, feeder F16 — akan datang) → review data merchant (foto, `deliveryConfig`) → SLA escalation (`batch.escalatedToAdmin: true` dari F21 — akan datang) berupa alert masuk untuk ditindak.
...
- Shell: panel admin (CS) memakai 430px seperti role lain — CS memakainya dari ponsel. Pengecualian lebar adalah urusan **Super Admin** (role terpisah, website penuh non-PWA, prefix `/superadmin`, belum dibangun). Saat dibangun, pengecualian lebar wajib didokumentasikan di `/documentation` (AGENTS.md §9) — bukan bocor diam-diam.
...
- **Dasar: `C-12`** — portal CS (approval tenant, blacklist COD, queue dispute). **Sebagian digantikan keputusan PO 2026-09-23**: konsol ini adalah panel admin/CS, sedangkan Super Admin role terpisah (`decision-irbid-mvp.md`). Plus PRD aktif §5C (deposit COD **3,50 JOD**) + `R-DISPUTE-01`. Tidak ada `R-SA-*` — **jangan dikarang**; skema draft entri 3/4/11/12/13/15 hanya dipakai untuk nama field
