...
Alamat customer dan gate coverage saat checkout: pin peta → zona (haversine ke dapur merchant, **Hijazi / Syimali ≤2 km**) → pilih alamat → gate coverage melawan `merchant.deliveryConfig` → kalau lolos ongkir tampil lalu zona ikut sebagai snapshot order. Dasar bisnis: `C-13` (2 zona Hijazi/Syimali, ≤2 km, per-merchant `is_active_hijazi` / `is_active_syimali`) + `C-05` + `C-07` (ongkir 100% merchant) — lihat `../BUSINESS.md`. Zona **A/B/C 600 m / 1,5 km / 2 km + tarif 5.000 / 9.000 / 13.000** di `src/data/merchant.ts` adalah **LEGACY PRD lama**, bukan ketentuan aktif. Repo ini front-end saja — form alamat adalah layar mock (AGENTS.md §1).
...
- Ongkir dan zona dihitung di luar repo (server-side); layar hanya menampilkan angka mock — jangan hitung sendiri di JSX.
...
- Angka zona A/B/C dan tarif di `src/data/merchant.ts` (`DELIVERY_ZONES`, `MAX_DELIVERY_METERS`) = **LEGACY PRD lama** — bukan ketentuan aktif PRD Irbid; jangan dikutip sebagai aktif. Kode repo masih memakai ini, jadi saat layar disentuh: tampilkan Hijazi/Syimali, bukan A/B/C.
...
- Kode `src/data/merchant.ts` — `DELIVERY_ZONES` (A/B/C, **LEGACY**), `MAX_DELIVERY_METERS`, `DEFAULT_NEW_ADDRESS_PIN`, `zoneFor()`, `haversineMeters()`, `deliveryFeeFor()`, `isDeliverable()`
...
Terakhir diperbarui: 2026-09-22 — **ditambah kartu bisnis di dalam diagram** (masalah · aktor/nilai · skenario) supaya skenario terbaca tanpa buka README; zona Hijazi/Syimali ≤2 km (`C-13`), A/B/C tetap disebut legacy. Validate 9/9 pass, deliver exit 0 (spec `f33c1410`, artifact `52c58941`), visual-check pass (1440x900, 2048x1320 light + dark, overflow 0).
