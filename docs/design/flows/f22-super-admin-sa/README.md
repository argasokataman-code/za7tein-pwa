...
- **Dossier merchant: rangkuman plus kewenangan status tenant.** `/superadmin/users/merchant/:id` merangkum satu merchant: identitas usaha, keadaan tenant (status, deposit, zona aktif, COD bermasalah), kurir yang dipekerjakan, sengketa, uang di ledger, dan alert SLA. **Aksi yang ada:** suspend dan aktifkan kembali, lewat izin `tenant.status` (grup platform, jadi `sa_ops` tidak punya), dan tercatat di audit trail **atas nama operator SA yang aktif** — bukan CS, karena tombolnya sama dengan tombol CS tetapi pelakunya berbeda. **Aksi yang tidak ada:** blacklist COD (menandai dua sisi sekaligus, merchant + `riskFlag` customer) dan approval onboarding tenant, keduanya tetap kerja panel CS (`/admin/merchants`). Batas itu dijaga kode, bukan hanya dokumen: `reinstateMerchant` hanya menerima status `suspended`.
